  ...
  return processDirAsync(src)
    ...
    .then(metas => {
      ...
      Object.keys(metas).forEach(fname => {
        ...
        info(log, ' Data mine  ' + m.name + ' will be written in the DB. ');
    ...
    .catch((e) => {
      if (e.code === 'ENOENT') {
        warn(log, 'Report module metadata directory not found ' + src);
  ...
  return processDirAsync(src)
    ...
    .then(metas => {
      ...
      Object.keys(metas).forEach(fname => {
        ...
        info(log, 'Data navigation node ' + meta.code + ' will be written in the DB.');
        p = p.then(() => dataSource.upsert(
          'ion_bi_nav',
          {
            [F.AND]: [
  ...
  return di(
    ...
    ['background'],
      ...
      function (scope) {
        ...
        info(scope.sysLog, 'Import report module meta from ' + pth);
