...
function StdMineBuilder(cOptions) {
  ...
  /**
   * Method obtains information about the latest data mine build
   * @param {String} mine
   * @param {String} name
        ...
        for (let ao in load.results[i].expr) {
          if (
            ...
            if (!Array.isArray(load.results[i].expr[ao])) {
              throw new Error('Syntax error in aggregation expression ' + JSON.stringify(load.results[i]));
            }
      ...
      .then(
        (info) => {
          ...
          if (!Array.isArray(source.load) || !source.load.length) {
            throw new Error('Not described the logic of the formation of the data source.');
          }
